...
\begin{algorithm}
	...
	\Repita{todos os times reais sejam associados aos times abstratos} {
		Seja $T1$ o próximo time real a ser associado a um time abstrato\\
		e seja $T2$ o time, cuja cidade é mais próxima à cidade de $T1$.\\
		\eSe{$T2$ já foi associado} {
			Encontre o primeiro par $(a,b)$ de times abstratos, tal que $a$ esteja\\
			associado ao time $T2$ e $b$ ainda não esteja associado a nenhum time real.\\
			$b \leftarrow T1$ }
